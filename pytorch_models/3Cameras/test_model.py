...
class CarlaGame(object):
    ...
    def _print_player_measurements(self, player_measurements):
        ...
        #STEER=np.argmax(model.predict(np.array([ARR/255])))-1
        image = CENTER[65:-25, :, :]
        ...
        STEER=model(image).view(-1).item()

        if abs(player_measurements.autopilot_control.steer)>0.09:
            print(f"The difference is {abs(STEER-player_measurements.autopilot_control.steer)}")
